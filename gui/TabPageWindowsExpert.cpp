...
TabPageWindowsExpert::TabPageWindowsExpert( window wd,MainFormBase* parentForm )
	:
...
TabPageWindowsExpert::~TabPageWindowsExpert()
{
}


void TabPageWindowsExpert::InitTabPage()
{
	// setup gtc

	m_gtc.Initialize( TABPAGE_WINDOWSEXPERT_FILEPATH_BASE );
	m_gtc.FetchRequests();
...
void TabPageWindowsExpert::UpdateStringsWidgets()
{
	std::string basePath = TABPAGE_WINDOWSEXPERT_FILEPATH_BASE + GET_CURLANGUAGE_STR + "/";
	...
	*/
	...
	m_vComboBoxes[ 0 ]->events().selected( [ this,labelStrings ]( const nana::arg_combox& acmb ) mutable
	{
		...
		//std::cout << acmb.widget.option() << std::endl;
...
void TabPageWindowsExpert::UpdateStringsModalForms()
{
}
